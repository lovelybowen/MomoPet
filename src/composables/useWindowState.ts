...
import { clampWindowPosition, isPositionOnMonitor } from '@/utils/window'
...
export function useWindowState() {
  ...
  const clampToMonitor = useDebounceFn(async () => {
    if (label !== WINDOW_LABEL.MAIN || !petStore.window.keepInScreen) return
    ...
    const { x: clampedX, y: clampedY } = clampWindowPosition(
      windowPos,
      windowSize,
      { position: monitorPos, size: monitorSize },
    )
  ...
  watch(() => petStore.window.keepInScreen, clampToMonitor)
  ...
  const restoreState = async () => {
    ...
    if (isNumber(x) && isNumber(y)) {
      ...
      const monitor = monitors.find(monitor => isPositionOnMonitor({ x, y }, monitor))
